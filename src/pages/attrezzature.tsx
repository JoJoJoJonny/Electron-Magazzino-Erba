...
// Definisco l'interfaccia dei dati del attrezzatura
interface AttrezzaturaInputs {
...
// Interfaccia che rappresenta il Attrezzatura completo letto dal DB (include l'ID interno di SQLite)
// L'ID interno sarà usato per l'UPDATE della riga.
export interface AttrezzaturaRecord extends AttrezzaturaInputs {
...
interface DatalistInputProps {
    id: keyof AttrezzaturaInputs;
    label: string;
    register: UseFormRegister<AttrezzaturaInputs>;
...
const AddEquipmentModal: React.FC<AddEquipmentModalProps> = ({ isOpen, onClose, availableDdt, availableCodArticolo }) => {
    const { register, handleSubmit, reset, formState: { isSubmitting, errors } } = useForm<AttrezzaturaInputs>();
    const [statusMessage, setStatusMessage] = useState('');

    if (!isOpen) return null;

    const onSubmit: SubmitHandler<AttrezzaturaInputs> = async (data) => {
        setStatusMessage('Salvataggio in corso...');

        try {
            const result = await window.ipcRenderer.invoke('db-insert-attrezzatura', data);

            if (result && result.success) {
                setStatusMessage('Attrezzatura inserita con successo!');
                reset();
                onClose();
            } else {
    ...
    return (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-lg shadow-2xl w-full max-w-lg">
                <h2 className="text-2xl font-bold mb-4 text-myColor">Aggiungi Nuova Attrezzatura</h2>
...
interface EditEquipmentModalProps {
    isOpen: boolean;
    onClose: () => void;
    equipmentToEdit: AttrezzaturaRecord; // Il attrezzatura da popolare e modificare
    forceRefresh: () => void;
...
const EditEquipmentModal: React.FC<EditEquipmentModalProps> = ({
    ...
    // Usiamo 'useForm' con il tipo AttrezzaturaInputs (che è l'output del form)
    const {
        register,
        handleSubmit,
        reset,
        ...
        watch
    } = useForm<AttrezzaturaInputs>({
        // Passiamo i valori del record per i default. React Hook Form li accetterà.
        defaultValues: {
            ddtCliente: equipmentToEdit.ddtCliente,
            codArticolo: equipmentToEdit.codArticolo,
            stoccaggio: equipmentToEdit.stoccaggio,
        }
    });
    const [statusMessage, setStatusMessage] = useState('');

    // Stato calcolato per sapere se i dati sono cambiati
    // Confrontiamo i valori correnti del form (formValues, tipo AttrezzaturaInputs)
    // con i valori iniziali (equipmentToEdit, tipo AttrezzaturaRecord)
    const formValues = watch();

    // Dobbiamo ciclare solo sulle chiavi che esistono in AttrezzaturaInputs
    const keysToCompare: (keyof AttrezzaturaInputs)[] = ['ddtCliente', 'codArticolo', 'stoccaggio'];

    // Controlla se almeno uno dei campi è diverso
    const isDirty = keysToCompare.some(key => {
        const formValue = formValues[key];
        const initialValue = equipmentToEdit[key as keyof AttrezzaturaRecord];
    ...
    // Effetto per aggiornare i valori del form se il attrezzatura da editare cambia
    // Questo è cruciale quando si seleziona un NUOVO attrezzatura per la modifica.
    useEffect(() => {
        if (equipmentToEdit && equipmentToEdit.rowid) {
...
// ... (il resto del componente continua normalmente)
    ...
    const onSubmit: SubmitHandler<AttrezzaturaInputs> = async (data) => {
        ...
        try {
            ...
            const result = await window.ipcRenderer.invoke('db-update-attrezzatura', {
                ...
                rowid: equipmentToEdit.rowid,
                // Passiamo i dati modificati, inclusa la nuova ddt
                ...data
            });

            if (result && result.success) {
                setStatusMessage('Attrezzatura aggiornato con successo!');
    ...
    return (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-lg shadow-2xl w-full max-w-lg">
                <h2 className="text-2xl font-bold mb-4 text-yellow-600">Modifica Attrezzatura ID Interno: {equipmentToEdit.rowid}</h2>
...
import TabellaAttrezzature from '@/components/tabellaAttrezzature';
...
export const Attrezzature = () => {
    ...
    // NUOVO STATO: Stato per tenere traccia del attrezzatura selezionato (per Modifica/Elimina)
    const [selectedEquipment, setSelectedEquipment] = useState<AttrezzaturaRecord | null>(null);
    ...
    // Funzione per gestire la selezione di un attrezzatura
    const handleEquipmentSelect = (equipment: AttrezzaturaRecord | null) => {
    ...
    const openAddModal = () => {
        setSelectedEquipment(null); // Assicurati che non ci siano selezioni attive
    ...
    // Ascolta l'evento 'refresh-attrezzature' dal processo principale (Electron)
    useEffect(() => {
        if (window.ipcRenderer) {
            ...
            const subscription: any = window.ipcRenderer.on('refresh-attrezzature', forceRefresh);
            ...
            return () => {
                ...
                window.ipcRenderer.off('refresh-attrezzature', subscription as IpcListenerSubscription);
            };
        }
    }, []);
    ...
    const forceRefresh = () => {
        ...
        setSelectedEquipment(null); // Deseleziona al refresh
        setMessage(null); // Pulisce il messaggio di stato
    };

    // NUOVO STATO INTERMEDIO per l'eliminazione.
    const handleDelete = () => {
        if (!selectedEquipment || !selectedEquipment.rowid) {
            setMessage({ text: "Nessun attrezzatura valida selezionata per l'eliminazione.", type: 'error' });
            return;
        }
    ...
    const confirmDelete = async () => {
        ...
        if (!selectedEquipment || !selectedEquipment.rowid) return;

        try {
            // Chiama la funzione IPC passando solo l'ID (ROWID)
            const result = await window.electronAPI.deleteAttrezzatura(selectedEquipment.rowid);

            if (result && 'error' in result) {
                setMessage({ text: `Errore durante l'eliminazione: ${result.error}`, type: 'error' });
            } else {
                setMessage({ text: result.message, type: 'success' });
                // Forza il ricaricamento della tabella e deseleziona il attrezzatura
                forceRefresh();
            }
        } catch (e) {
    ...
    return (
        <div className="p-6 space-y-6">
            ...
            <h1 className="text-3xl font-bold text-gray-800">Gestione Attrezzature</h1>
            ...
            <div className="flex items-center space-x-4">
                ...
                <div className="flex space-x-2">
                    ...
                    {/* Tasto Modifica (Abilitato solo se un attrezzatura è selezionato) */}
                    <button
                        onClick={openEditModal}
                        disabled={!selectedEquipment} // Disabilita se selectedEquipment è null
                        className={`flex items-center px-4 py-2 text-white font-semibold rounded-lg shadow-md transition 
                            ${selectedEquipment ? 'bg-yellow-500 hover:bg-yellow-600' : 'bg-yellow-300 opacity-50 cursor-not-allowed'}`}
                    >
                    ...
                    {/* Tasto Elimina (Abilitato solo se un attrezzatura è selezionato) */}
                    <button
                        onClick={handleDelete}
                        disabled={!selectedEquipment}
                        className={`flex items-center px-4 py-2 text-white font-semibold rounded-lg shadow-md transition 
                            ${selectedEquipment ? 'bg-red-600 hover:bg-red-700' : 'bg-red-300 opacity-50 cursor-not-allowed'}`}
                    >
            ...
            {/* 3. Visualizzazione della Tabella Attrezzature */}
            <div className="min-h-screen pt-4">
                {/* PASSAGGIO PROPS: selectedEquipment (per evidenziazione) e handleEquipmentSelect (per aggiornare lo stato) */}
                <TabellaAttrezzature
                    key={refreshKey}
            ...
            {/* Il Modale Modifica compare SOLO se un attrezzatura è selezionato */}
            {selectedEquipment && (
                <EditEquipmentModal
            ...
            {selectedEquipment && (
                <ConfirmationModal
                    ...
                    title="Conferma Eliminazione Attrezzatura"
                    message={`Sei sicuro di voler eliminare definitivamente il attrezzatura "${selectedEquipment.id}" (ROWID: ${selectedEquipment.rowid})? Quest'azione non può essere annullata.`}
